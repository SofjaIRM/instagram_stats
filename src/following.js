...
const previousFollowingFileName = getFollowsList(
  FOLLOWING_PATH,
  PREVIEWS_FILE_INDEX,
);
...
function getNewUsersWeFollow({ previousFollowingList, currentFollowingList }) {
  return currentFollowingList.filter(
    ({ id }) => !findUser(previousFollowingList, id),
  );
}

function getUsersStartingFollowingUs({
  previousFollowingList,
  currentFollowingList,
}) {
  return currentFollowingList.filter(({ id }) =>
    findUser(previousFollowingList, id),
  );
}

function getUsersStartingFollowingBack(lists) {
  return getUsersStartingFollowingUs(lists).filter(
...
async function startFollowingStatistics() {
  ...
  const usersNotFollowingUsBack =
    getUsersNotFollowingBack(currentFollowingList);

  // Users that started following back after we follow them
  const usersStartingFollwingBack =
    getUsersStartingFollowingBack(followingLists);
