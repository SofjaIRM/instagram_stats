...
function startFollowersStatistics() {
  ...
  const newFollowers = utils.sortBy(getNewFollowers(lists));
  const unfollowers = utils.sortBy(getUnfollowers(lists));
  const unfollowersWeFollow = utils.sortBy(getUnfollowersWeFollow(lastFollowingList, unfollowers))
